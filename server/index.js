...
const { getScores, postGoVerify } = require('./go-verify');
...
app.post('/user', (req, res) => {
  ...
  if (!username) {
    return res.status(500)
      .json({ error: 'Must provide username!' });
  }
...
app.get('/user/:userId', (req, res) => {
  if (!req.params.userId) {
    return res.status(500)
      .json({ error: 'Please do not forget to supply an user ID!' });
  }
...
app.post('/buy', (req, res) => {
  ...
  if (!purchases.length) {
    return res.status(403)
      .json({ error: 'Come on, add at least something into your cart!' });
  }
  const committedBuys = purchases.map(p => purchaseBread(userId, p.id, p.qty));
  return res.status(200)
    .json({ purchases: committedBuys, items: users.filter(u => u.id === userId)[0].items });
});
